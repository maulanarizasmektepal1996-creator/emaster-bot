...
def menu_content(user):
    ...
    nip = user[1]
    masked_nip = ("•" * max(0, len(nip) - 4)) + nip[-4:]
...
async def show_menu(message, user, edit=False):
    text, keyboard = menu_content(user)
    if edit:
        try:
...
async def otp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        rows = [[InlineKeyboardButton("➕ Tambah Aktivitas", callback_data="menu:add"),
                 InlineKeyboardButton("📊 Dashboard", callback_data="menu:progress")]]
        if storage.get_draft(update.effective_user.id):
            rows.insert(0, [InlineKeyboardButton("📝 Lanjutkan Draft", callback_data="menu:resume")])
